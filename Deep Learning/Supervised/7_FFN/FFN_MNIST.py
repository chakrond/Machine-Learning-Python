...
# data_train, data_test, y_train, y_test = train_test_split(data_ts, y_ts, test_size=test_size)
...
# # convert to PyTorch Datasets
# Dataset_train = TensorDataset(data_train, y_train)
# Dataset_test = TensorDataset(data_test*np.max(data), y_test)
...
optimTypes = ['SGD']
data_in = [[DataLoader_train, DataLoader_test]]

model_params_0 = data_in
model_params_1 = optimTypes
...
for (e, param_0) in enumerate(model_params_0):
    ...
    for (i, param_1) in enumerate(model_params_1):
        ...
        # dict
        input_dict = {
            'DataLoader_train': model_params_0[e][0],
            'DataLoader_test': model_params_0[e][1],
            }

        # result during training per each epoch
        _, model_losses_train, model_acc_ep_train, model_acc_ep_test, _ = FFN_model.trainModel(
            **input_dict,
            epochs=epochs,
            loss_function='NLL',
            comp_acc_test=True
