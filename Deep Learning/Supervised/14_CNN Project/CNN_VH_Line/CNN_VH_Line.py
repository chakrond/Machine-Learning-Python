...
for i in range(len(labels)):
    ...
    if randNum == 0:
        ...
        rndR = np.random.randint(5, imgSize-5)
        offR = np.random.randint(1, 3)
        rndR = np.arange(rndR-offR, rndR+offR)
        img[rndR, :] = 1
        ...
        rndC = np.random.randint(5, imgSize-5)
        offC = np.random.randint(1, 3)
        rndC = np.arange(rndC-offC, rndC+offC)
        img[:, rndC] = 1
        
        labels[i] = 1
        
    # store in tensors
    imgDat[i, 0, :, :] = torch.tensor(img)
...
for i, ax in enumerate(axs.flatten()):
    ...
    ax.imshow(G, vmin=0, vmax=1, cmap='turbo')
    ax.set_title(f'Class {labels[pic2show].int().item()}')
    ax.set_xticks([])
    ax.set_yticks([])

#%% divide independent set
data_ss, data_test_ind, y_ss, y_test_ind = train_test_split(imgDat, labels, test_size=0.2)
...
ax[1].imshow( np.reshape(X[errors[sample2show], :], (imgSize, imgSize)), cmap='turbo')
...
fig, axs = plt.subplots(3, 8, figsize=(20, 3))
fig.suptitle('True-Prediction')

# original images from teh ind test set
for i, axi in enumerate(axs.flatten()):
    
    axi.imshow(X[randId[i], 0, :, :], cmap='turbo')
    axi.axis('off')
    # axi.set_title(f'T:{labels[y[randId[i]]]} P:{labels[predicitons[randId[i]]]}')
    label_t = f'T:{labels[y[randId[i]].int().item()].int().item()} P:{labels[predictions[randId[i]].int().item()].int().item()}'
...
for i in range(imgSize):
    ...
    # plot
    a[i+imgSize].imshow(imgs[i+imgSize, 0, :, :])
    a[i+imgSize].set_title(f'{slopes[i]:.2f}')
...
# convert to tensor
randIdx = np.random.permutation(range(imgs.shape[0]))
img_ts = torch.tensor(imgs[randIdx, :, :, :].astype(np.float32))
...
X = img_ts.clone()
# y = y_test_ind_ts
predictions, _ = CNN_model.predict(X)
predictions_raw = predictions.clone()
predictions = (predictions > 0.5).float() # binary
...
for i, axi in enumerate(axs.flatten()):
    ...
    axi.imshow(X[randId[i], 0, :, :])
    axi.axis('off')
    # axi.set_title(f'T:{labels[y[randId[i]]]} P:{labels[predicitons[randId[i]]]}')
    title = ['Horizontal', 'Vertical']
    label_t = f'P:{title[labels[predictions[randId[i]].int().item()].int().item()]}'
    axi.text(16, 0, label_t, ha='center', fontweight='bold', color='k', backgroundcolor='y')
