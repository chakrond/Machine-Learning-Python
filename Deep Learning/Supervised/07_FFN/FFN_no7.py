...
# get rid of 7s
data7 = dataNorm[labels==7]
labels7 = labels[labels==7]
dataNo7 = dataNorm[labels!=7]
labelsNo7 = labels[labels!=7]

data_ts = torch.tensor(dataNo7).float()
y_ts = torch.tensor(labelsNo7).long()
data_ts_7 = torch.tensor(data7).float()
y_ts_7 = torch.tensor(labels7).long()
...
train_data = TensorDataset(data_ts, y_ts)
test_data  = TensorDataset(data_ts_7, y_ts_7)

# translate into dataloader objects
DataLoader_train = DataLoader(train_data, batch_size=b_size, shuffle=True, drop_last=True)
DataLoader_test  = DataLoader(test_data, batch_size=test_data.tensors[0].shape[0])
...
plt.legend([f'Lr {param}' for param in model_params_1] + ['Mean'])
