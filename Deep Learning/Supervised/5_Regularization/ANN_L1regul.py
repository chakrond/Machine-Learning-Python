...
class ANN_Class(nn.Module):
    ...
    def __init__(self, 
        ...
        # ---------------------------------------
        if p_lambda1 == None:
            self.optimizer = torch.optim.SGD(self.parameters(), lr=learning_rate, weight_decay=p_lambda2) # stochastic gradient
        if p_lambda1 != None:
        ...
        for epoch in range(epochs):
            ...
            for X_batch_train, y_batch_true_train in DataLoader_train:
                ...
                if self.L1 != None:
                    ...
                    # add to loss term
                    loss = loss + self.L1*L1_term/self.nWeights
