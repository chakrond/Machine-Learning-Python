...
#%% delete random of data (for prediction missing value study case)

rdIdx = np.random.choice(np.arange(len(data_z)), 10)
storeVal_x = data_z.drop(columns=['residual sugar']).loc[rdIdx, :].values
storeVal_y = data_z.loc[rdIdx, 'residual sugar'].values
data_z.loc[rdIdx, 'residual sugar'] = float("nan")
