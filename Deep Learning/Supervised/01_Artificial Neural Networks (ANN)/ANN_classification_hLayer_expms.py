...
def ANNclf_model(data, y_true, p_lr, n_hLayer):
    ...
    ANNclf = nn.Sequential(
        nn.Linear(4, n_hLayer),         # input layer nn.Linear(in_features, out_features)
        nn.ReLU(),                      # activation function
        nn.Linear(n_hLayer, n_hLayer),  # hidden layers
        nn.ReLU(),                      # activation function
        nn.Linear(n_hLayer, 3),         # output layer(input=64, output=3)
...
model_params_name = ['Hidden Layers']
...
for e in range(n_exprms):
    ...
    for (i, param) in enumerate(model_params):
        ...
        model_y_pred, model_predicitons, model_losses, model_acc, model_acc_ep, _  = ANNclf_model(data, labels, lr, param)
