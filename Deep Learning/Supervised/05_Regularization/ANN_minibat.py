...
# DataLoader_train = DataLoader(Dataset_train, shuffle=True, batch_size=p_batch_size) # drop_last=True, drop the last batch if it's not full batch
...
for (e, param_0) in enumerate(model_params_0):
    ...
    for (i, param_1) in enumerate(model_params_1):
        ...
        # dataLoader
        DataLoader_train = DataLoader(Dataset_train, shuffle=True, batch_size=int(param_1))
