...
for i in range(len(X_data_noise)):
    ...
    rndR = np.random.randint(0, imgSize)
    rndR = np.arange(rndR-1, rndR+1)
    rndC = np.random.randint(0, imgSize)
    rndC = np.arange(rndC-1, rndC+1)

    if np.random.randint(0, 3) == 0:
        X_data_noise[i, :].view(imgSize, imgSize)[rndR, :] = 1
    elif np.random.randint(0, 3) == 1:
        X_data_noise[i, :].view(imgSize, imgSize)[:, rndC] = 1
    else:
        X_data_noise[i, :].view(imgSize, imgSize)[rndR, :] = 1
        X_data_noise[i, :].view(imgSize, imgSize)[:, rndC] = 1
...
# sample to show
nPic = 10
randId = np.random.choice(len(X_data_noise), nPic, replace=False)

# run the model through for the test data
X = X_data_noise[randId, :, :, :].float()
predictions, _ = CNN_model.predict(X, X)
# predicitons = torch.sigmoid(predictions)

fig, axs = plt.subplots(2, nPic, figsize=(15, 3))
fig.suptitle('Original-Autoencoder with mean')

# original images from teh ind test set
for axi in range(len(randId)):
    axs[0, axi].imshow(X[axi, 0, :, :], cmap='turbo', vmin=0, vmax=1)
    axs[0, axi].axis('off')
    axs[0, axi].set_title(f'u={np.round(torch.mean(X[axi, 0, :, :]).item(), 3)}')
    
    axs[1, axi].imshow(predictions[axi, 0, :, :], cmap='turbo', vmin=0, vmax=1)
    axs[1, axi].axis('off')
    axs[1, axi].set_title(f'u={np.round(torch.mean(predictions[axi, 0, :, :]).item(), 3)}')
...
X = torch.tensor(imgDat[randId, :]).float()
predictions_woc, _ = CNN_model.predict(X, X)
predictions_woc = torch.sigmoid(predictions_woc)


# plot all
fig = plt.figure(constrained_layout=True)
fig.suptitle('Gauss CNN_AutoEncoder')
...
for i, idx in enumerate(randId):
    axes[0][i].imshow(data_train_ts[idx, :].view(imgSize, imgSize).detach(), cmap='turbo')
    axes[1][i].imshow(predictions_woc[i].view(imgSize, imgSize).detach(), cmap='turbo')
    axes[2][i].imshow(X_data_noise[idx, :].view(imgSize, imgSize).detach(), cmap='turbo')
    axes[3][i].imshow(predicitons[i].view(imgSize, imgSize).detach(), cmap='turbo')
