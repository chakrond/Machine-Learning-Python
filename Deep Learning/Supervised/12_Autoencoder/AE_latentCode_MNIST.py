...
# run the model through the data
X = torch.tensor(dataNorm).float()
predictions, _ = AE_model.predict(X, X)
predicitons = torch.sigmoid(predictions)

# data for ploting
dat_plot = AE_model.fwLat.detach().numpy()

fig, ax = plt.subplots(1, 2, figsize=(15, 5))

# plot histogram
val, bins, patches = ax[0].hist(dat_plot.flatten(), 100)
ax[0].set_xlabel('Latent activation value')
ax[0].set_ylabel('Count')
ax[0].set_ylim([0, 8000])
ax[0].set_title('Histogram of value in latent units')

# get colormap
cm = plt.cm.get_cmap('turbo')
# normalize values
vmax = 5000 
valNorm = (val-val.min()) / (vmax-val.min())
for c, p in zip(valNorm, patches):
    plt.setp(p, 'facecolor', cm(c))
plt.show()

# plot data image
img = ax[1].imshow(dat_plot, aspect='auto', vmin=0, vmax=10, cmap='turbo')
ax[1].set_xlabel('Latent node')
ax[1].set_ylabel('Image number')
ax[1].set_title('Latent node activations')
ax[1].set_xticks(np.arange(dat_plot.shape[1]))
ax[1].set_xticklabels(np.arange(dat_plot.shape[1]), rotation=0)
fig.colorbar(img, ax=ax[1])

#%% node activation with labels

labelAct = np.zeros((dat_plot.shape[1], len(np.unique(labels))))  
countAct = np.zeros((dat_plot.shape[1], len(np.unique(labels))))

# threshold count
tsh = 10
# convert to [0, 1]
con_dat_plot = np.zeros(dat_plot.shape)
con_dat_plot[dat_plot>10] = 1

for i in np.unique(labels).astype(dtype=int):
    
    imgIdx = np.where(labels==i)
    
    # average value of each label
    labelAct[:, i] = np.mean(dat_plot[imgIdx, :], axis=1)
    countAct[:, i] = np.sum(con_dat_plot[imgIdx, :], axis=1)


fig, ax = plt.subplots(1, 2, figsize=(15, 5))

# plot data image
img = ax[0].imshow(labelAct, aspect='auto', cmap='turbo', vmax=10)
ax[0].set_xlabel('Labels')
ax[0].set_xticks(np.arange(labelAct.shape[1]))
ax[0].set_xticklabels(np.arange(labelAct.shape[1]), rotation=0)
ax[0].set_ylabel('Latent node')
ax[0].set_yticks(np.arange(labelAct.shape[0]))
ax[0].set_yticklabels(np.arange(labelAct.shape[0]), rotation=0)
ax[0].set_title('Latent average values')
fig.colorbar(img, ax=ax[0])

# plot histogram
img = ax[1].imshow(countAct, aspect='auto', cmap='turbo', vmax=500)
ax[1].set_xlabel('Labels')
ax[1].set_xticks(np.arange(countAct.shape[1]))
ax[1].set_xticklabels(np.arange(countAct.shape[1]), rotation=0)
ax[1].set_ylabel('Latent node')
ax[1].set_yticks(np.arange(countAct.shape[0]))
ax[1].set_yticklabels(np.arange(countAct.shape[0]), rotation=0)
# ax[1].set_ylim([0, 5000])
ax[1].set_title('Histogram of value in latent units')
fig.colorbar(img, ax=ax[1])


# # plot 3d surface
# X = np.arange(dat_plot.shape[1])
# Y = np.arange(dat_plot.shape[0])
# X, Y = np.meshgrid(X, Y)
# fig, ax = plt.subplots(subplot_kw={"projection": "3d"})
# surf = ax.plot_surface(X, Y, dat_plot, cmap='turbo',linewidth=0)

#%% Dimensional reduction

pcaData = PCA(n_components=15).fit(dataNorm) # 15 components to match latent
pcaLatent = PCA().fit(dat_plot)

# plot 
fig, ax = plt.subplots(1, 1, figsize=(15, 5))

# plot PCA scree plot, the Percentage of variance explained by each of the selected components.
ax.plot(100*pcaData.explained_variance_ratio_, 's-', label='Data PCA')
ax.plot(100*pcaLatent.explained_variance_ratio_, 'o-', label='Latent PCA')
...
scoresData = pcaData.fit_transform(dataNorm)
scoresLatent = pcaLatent.fit_transform(dat_plot)

# plot the data for the first 2 dimensions
fig,ax = plt.subplots(2, 1, figsize=(15, 5))
plt.subplots_adjust(left=None, bottom=None, right=None,
                    top=None, wspace=None, hspace=0.25)

for lab in np.unique(labels).astype(dtype=int):
    ax[0].plot(scoresData[labels==lab, 0], scoresData[labels==lab, 1], 'o', markersize=3, alpha=0.4)
    ax[0].set_xlabel('PC1 projection')
    ax[0].set_ylabel('PC2 projection')
    ax[0].legend(range(10))
    
    ax[1].plot(scoresLatent[labels==lab, 0], scoresLatent[labels==lab, 1], 'o', markersize=3, alpha=0.4)
    ax[1].set_xlabel('PC1 projection')
    ax[1].set_ylabel('PC2 projection')
    ax[1].legend(range(10))

ax[0].set_title('PCA of data')
ax[1].set_title('PCA of latent')
