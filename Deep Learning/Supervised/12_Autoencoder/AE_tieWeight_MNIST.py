...
# copy tensors
X_data_noise = data_train_ts.clone().detach()

# add noise occlusion
for i in range(len(X_data_noise)):

    rndR = np.random.randint(0, 28)
    rndR = np.arange(rndR-1, rndR+1)
    rndC = np.random.randint(0, 28)
    rndC = np.arange(rndC-1, rndC+1)

    if np.random.randint(0, 3) == 0:
        X_data_noise[i, :].view(28, 28)[rndR, :] = 1
    elif np.random.randint(0, 3) == 1:
        X_data_noise[i, :].view(28, 28)[:, rndC] = 1
    else:
        X_data_noise[i, :].view(28, 28)[rndR, :] = 1
        X_data_noise[i, :].view(28, 28)[:, rndC] = 1
...
X = X_data_noise[randId, :].clone().detach()
predictions, _ = AE_model.predict(X, X)
predicitons = torch.sigmoid(predictions)

# get prediction without occlusion
X = torch.tensor(dataNorm[randId, :]).float()
predictions_woc, _ = AE_model.predict(X, X)
predictions_woc = torch.sigmoid(predictions_woc)
...
subfigs[1].suptitle('Denoise without occlusion')
subfigs[2].suptitle('Added occlusion')
subfigs[3].suptitle('Denoise occlusion')

# create 1x5 subplots per subfig
nSubplt = 5
axes = [ax.subplots(nrows=1, ncols=nSubplt) for ax in subfigs]

for i, idx in enumerate(randId):
    axes[0][i].imshow(data_train_ts[idx, :].view(28, 28).detach(), cmap='gray')
    axes[1][i].imshow(predictions_woc[i].view(28, 28).detach(), cmap='gray')
    axes[2][i].imshow(X_data_noise[idx, :].view(28, 28).detach(), cmap='gray')
    axes[3][i].imshow(predicitons[i].view(28, 28).detach(), cmap='gray')
    
    
    for z in range(4):
        axes[z][i].set_xticks([]), axes[z][i].set_yticks([])
        
plt.show()

#%% correlation check

# correlation plot between original and denoised with occlusion
...
for i, idx in enumerate(randId):
    # filter zeros value in the picture
    # boolean vector for pixels>0 (with tolerance)
    nonZeroPixels = (data_train_ts[idx, :]>tol) & (predicitons[i]>tol)
    corl = np.corrcoef(data_train_ts[idx, nonZeroPixels], predicitons[i][nonZeroPixels])
    ax[0].plot(data_train_ts[idx, nonZeroPixels], predicitons[i][nonZeroPixels], '.', markersize=7,
             label=f'Pic No.{i}, coef={np.round(corl[0, 1], 2)}')

ax[0].legend()
ax[0].set_title('Non-Zero Pixels Correlation(Original-Denoised with occulsion)')


# correlation plot between original and denoised result without occulsion
corl_0 = np.zeros(len(randId))
corl_1 = np.zeros(len(randId))
for i, idx in enumerate(randId):
    # filter zeros value in the picture
    # boolean vector for pixels>0 (with tolerance)
    nonZeroPixels_0 = (data_train_ts[idx, :]>tol) & (predictions_woc[i]>tol)
    corl_0[i] = np.round(np.corrcoef(data_train_ts[idx, nonZeroPixels_0], predictions_woc[i][nonZeroPixels_0])[0, 1], 2)
    
    nonZeroPixels_1 = (data_train_ts[idx, :]>tol) & (predictions[i]>tol)
    corl_1[i] = np.round(np.corrcoef(data_train_ts[idx, nonZeroPixels_1], predictions[i][nonZeroPixels_1])[0, 1], 2)


ax[1].plot(corl_0, 'o', markersize=10, label='Original-Without occlusion')
ax[1].plot(corl_1, 'o', markersize=10, label='Original-With occlusion')
ax[1].legend()
ax[1].set_title('Correlation of denoised')