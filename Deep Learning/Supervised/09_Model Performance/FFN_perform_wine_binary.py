...
#*****create a new column for boolean quality [0, 1, 2]
data_z['boolQuality'] = 0
data_z['boolQuality'][data['quality']>5] = 1 # wine quality > 5 will assigned as 1
data_z[['quality', 'boolQuality']]


# inspect after booleanized

fig, ax = plt.subplots(3, 1, figsize=(17, 4))
plt.subplots_adjust(left=None, bottom=None, right=None, top=None, wspace=None, hspace=0.6)

# inspect values
value_name = 'boolQuality'

fig.suptitle(f'{value_name} Distribution (booleanized)', fontweight='bold')
...
y_test_ind_ts = torch.tensor(y_test_ind).float() # .long() (integer and for cross-entropy, multi class)
y_test_ind_ts = y_test_ind_ts.reshape((-1, 1)).float() # .float() and reshape only for binary
...
y_ts = torch.tensor(y_ss).float() # .long() (integer and for cross-entropy, multi class)
y_ts = y_ts.reshape((-1, 1)).float() # .float() and reshape only for binary
...
plt.bar(np.unique(y_data), predictions[sample2show])
...
ax[0].bar(np.unique(y_data), predictions[errors[sample2show]])
...
predictions, _ = FFN_model.predict(X, y)
# predicitons = torch.log_softmax(predictions, axis=1)
# predictions = torch.argmax(predicitons, axis=1)
...
# predictions_train = torch.softmax(predictions_train, axis=1)
# predictions_train = torch.argmax(predictions_train, axis=1)
predictions_train = (predictions_train > 0.5).float()
train_score = skmScore(y_true_train, predictions_train)


X, y_true_test = data_test_ind_ts, y_test_ind_ts
predictions_test, _ = FFN_model.predict(X, y_true_test)
# predicitons_test = torch.softmax(predictions_test, axis=1)
# predictions_test = torch.argmax(predicitons_test, axis=1)
predictions_test = (predictions_test > 0.5).float()
score_test = skmScore(y_true_test, predictions_test)


# plot model performance score

fig, ax = plt.subplots(1, 3, figsize=(15, 5))
fig.suptitle('Performance score of trained model (binary classification)')

rects1 = ax[0].bar(np.arange(4)-0.1, train_score.values(), 0.5)
rects2 = ax[0].bar(np.arange(4)+0.1, score_test.values(), 0.5)
ax[0].set_xticks(np.arange(4), train_score.keys())
ax[0].set_ylim([0.4, 1])
...
skm.ConfusionMatrixDisplay.from_predictions(y_true_train, predictions_train, ax=ax[axid], colorbar=False)
ax.flatten()[axid].title.set_text('Train set')
plt.axis("on")

axid = 2
fig.add_subplot(ax.flatten()[axid])
skm.ConfusionMatrixDisplay.from_predictions(y_true_test, predictions_test, ax=ax[axid], colorbar=False)
