...
def skmScore(true, prediction):
    ...
    data_metric = np.zeros(4)
    
    data_metric[0] = skm.accuracy_score(true, prediction)
    data_metric[1] = skm.precision_score(true, prediction)
    data_metric[2] = skm.recall_score(true, prediction)
    data_metric[3] = skm.f1_score(true, prediction)
...
data_z['boolQuality'] = 1
data_z['boolQuality'][data['quality']<5] = 0 # wine quality < 5 will assigned as 0
data_z['boolQuality'][data['quality']>=6] = 2
...
# y_test_ind_ts = y_test_ind_ts.reshape((-1, 1))
...
y = DataLoader_train.dataset.tensors[1]
predictions_train, _ = FFN_model.predict(X, y)
predictions_train = torch.log_softmax(predictions_train, axis=1)
predictions_train = torch.argmax(predictions_train, axis=1)
train_score = skm.accuracy_score(y, predictions_train)


X, y = data_test_ind_ts, y_test_ind_ts
predictions_test, _ = FFN_model.predict(X, y)
predicitons_test = torch.log_softmax(predictions_test, axis=1)
predictions_test = torch.argmax(predicitons_test, axis=1)
score_test = skm.accuracy_score(y, predictions_test)
