...
data = np.random.randint(low=-10, high=10, size=(2000, 2))
data = np.concatenate((data, np.sum(data, axis=1).reshape(-1, 1)), axis=1)

cate = np.zeros(len(data))
for i, s in enumerate(np.unique(data[:, 2])):
...
for col in range(data.shape[1]):
    ...
    data_z[:, col] = (data[:, col] - meanval) / stdev

# z-score by scipy
# data_z[features] = data_z[features].apply(stats.zscore) # pandas dataframe
y_data_z = stats.zscore(y_data) # numpy
...
corrcoef_mat = np.corrcoef(data.T)
unique, counts = np.unique(data, return_counts=True)
plt.imshow(corrcoef_mat)
plt.xticks(range(len(unique)), labels=unique, rotation=90)
plt.yticks(range(len(unique)), labels=unique)
plt.colorbar()
...
data_ss, data_test_ind, y_ss, y_test_ind = train_test_split(data_z, y_data, test_size=0.1)
