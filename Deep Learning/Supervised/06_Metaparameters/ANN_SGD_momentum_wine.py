...
class ANN_Class(nn.Module):
    ...
    def __init__(self, 
        ...
        # ---------------------------------------
        if w_decay == 'L2':
            self.optimizer = torch.optim.SGD(self.parameters(), lr=learning_rate, weight_decay=p_lambda, momentum=p_momentum) # stochastic gradient
        else:
            self.optimizer = torch.optim.SGD(self.parameters(), lr=learning_rate, momentum=p_momentum)
    ...
    def forward(self, x):
        ...
        for i in range(self.n_hLayer):
            ...
            if self.actLib == 'torch':
                # hidden layer
                x = actFun( self.layers[f'hidden{i}'](x) ) # x = F.relu( self.layers[f'hidden{i}'](x) )
...
data_ts = torch.tensor(data_z[features.drop(y_feature)].values).float()
y_ts = torch.tensor(data_z[y_feature].values).float()
...
data_train,data_test, y_train,y_test = train_test_split(data_ts, y_ts, test_size=model_test_size)
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title('Loss by Epoch(Train set)')
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title('Loss by Epoch(test set)')
plt.legend([f'momentum {param}' for param in model_params_1] + ['Mean'])


#%% show predictions and real values **from the last model
yHatTrain, _ = ANNclf_model.predict(data_train, y_train)
yHatTest, _  = ANNclf_model.predict(data_test, y_test)
...
plt.figure()
# plt.subplot(1, 2, 2)
plt.plot(yHatTrain.detach(), y_train,'ro')
plt.plot(yHatTest.detach(), y_test,'b^')
...
#%% plot experiment parameters (loss)

# average over last 100 epochs
a_p = np.zeros((2, len(model_params_1)))
for i, lis in enumerate(list_losses_train_params_0):
...
ax[1].set_ylabel('Train-test difference (MSE loss)')
...
#%% plot experiment parameters (acc)
