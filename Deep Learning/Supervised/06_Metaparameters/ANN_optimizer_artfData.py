...
for (e, param_0) in enumerate(model_params_0):
    ...
    for (i, param_1) in enumerate(model_params_1):
        ...
        y_pred, _ = ANNclf_model.predict(data_ts, y_ts)
...
for (li, pr1) in zip(y_pred_model, model_params_1):

    yHat = torch.tensor(li)
    ...
    plt.xlabel('Group')
    plt.ylabel('Accuracy (%)')
    plt.title(f'Final accuracy = {totalAcc:.2f}% - {pr1}')
    
    
    sm = nn.Softmax(dim=1)
    fig = plt.figure()
    fig.suptitle(f'Overall performance of model prediction - {pr1}', fontweight='bold')
