...
# oversampling
dataNorm_ovs = np.concatenate((dataNorm, dataNorm))
labels_ovs = np.concatenate((labels, labels))

data_ts_ovs = torch.tensor(dataNorm_ovs).float()
y_ts_ovs = torch.tensor(labels_ovs).long()
...
# oversampling 
DataLoader_train_ovs, DataLoader_test_ovs = dSplit(data_ts_ovs, y_ts_ovs, model_test_size=test_size, p_batch_size=b_size)
...
optimTypes = ['Adam']
dataLoaderSet = [[DataLoader_train, DataLoader_test], [DataLoader_train_ovs, DataLoader_test_ovs]]
