...
# show the data
plt.figure()
plt.plot(data_np[labels_np.ravel()==0, 0], data_np[labels_np.ravel()==0, 1], 'bs')
plt.plot(data_np[labels_np.ravel()==1, 0], data_np[labels_np.ravel()==1, 1], 'ko')
plt.show()
...
keys = np.unique(labels_np)
counts = []
for l in keys:
    n = len(labels_np[labels_np==l])
    counts.append(n)

plt.bar(list(keys), counts)
...
labels_ts = torch.tensor(labels_np).float()
labels_ts = labels_ts.reshape((-1, 1))
...
for (e, param_0) in enumerate(model_params_0):
    ...
    for (i, param_1) in enumerate(model_params_1):
        ...
        ANNclf_model = ANN_Class(feature_in=2, 
                                 feature_out=1, 
                                 n_hUnit=16, 
                                 n_hLayer=param_0, 
                                 dropout_rate=dr, 
...
plt.legend([f'batch norm = {i}'for i in model_params_1])
plt.show()
