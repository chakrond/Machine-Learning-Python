...
for (i, clf) in enumerate(classifiers):
    ...
    y_pred = clf.predict(kernel_pca_X_test)
    ConfusionMatrixDisplay.from_predictions(y_test, y_pred, ax=axes.flatten()[i], colorbar=False)
...
X_set, y_set = kernel_pca_X_train, y_train
...
X_set, y_set = kernel_pca_X_test, y_test
X1, X2 = np.meshgrid(np.arange(start = X_set[:, 0].min() - 1, stop = X_set[:, 0].max() + 1, step = 0.5),
                     np.arange(start = X_set[:, 1].min() - 1, stop = X_set[:, 1].max() + 1, step = 0.5))
